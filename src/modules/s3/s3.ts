...
export class S3 {
  ...
  async download (key: S3Key): Promise<string> {
    const command = new GetObjectCommand({
      Bucket: this.bucketName,
      Key: this.prependEnvKey(key)
    })
  ...
  async upload (key: S3Key, content: Buffer): Promise<void> {
    const command = new PutObjectCommand({
      Bucket: this.bucketName,
      Key: this.prependEnvKey(key),
      Body: content,
  ...
  async uploadStream (key: S3Key, stream: Readable): Promise<void> {
  ...
  async list (key: S3Key): Promise<ListObjectsV2Output['Contents']> {
  ...
  private prependEnvKey (key: S3Key): S3Key {
    const env: string = this.configService.get('NODE_ENV', 'local')
    return `${env}/${key}` as S3Key
  }
}
